...
GLuint LoadCubeMap(const char* filename[6])
{
	...
	for (int i = 0; i < 6; i++) {
		cubemap_image[i] = stbi_load(filename[i], &w, &h, &c, STBI_rgb_alpha);
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA8, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, cubemap_image[i]);

		if (cubemap_image[i] != nullptr) {
			stbi_image_free(cubemap_image[i]);
		}
	}
...
void Initialize()
{
	...
	const char* chemins[6] = { "../data/envmaps/test_px.png","../data/envmaps/test_nx.png","../data/envmaps/test_py.png","../data/envmaps/test_ny.png","../data/envmaps/test_pz.png","../data/envmaps/test_nz.png" };
	EnvMapTexture = LoadCubeMap(chemins);
...
void Display(GLFWwindow* window)
{
	...
	// ----
	glActiveTexture(GL_TEXTURE1); // on la bind sur la texture unit #0
	glBindTexture(GL_TEXTURE_CUBE_MAP, EnvMapTexture);

	const int samplercube =
		glGetUniformLocation(lightShader.GetProgram(), "u_envmapSampler");
	glUniform1i(samplercube, 1);
